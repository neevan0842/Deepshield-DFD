...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print(
                "=> loaded checkpoint '{}' (epoch {}, bce_best {})".format(
    ...
    if conf["fp16"]:
        model, optimizer = amp.initialize(
            model, optimizer, opt_level=args.opt_level, loss_scale="dynamic"
        )
...
def evaluate_val(
    args, data_val, bce_best, model, snapshot_name, current_epoch, summary_writer
):
    print("Test phase")
    model = model.eval()
    ...
    if args.local_rank == 0:
        ...
        if bce < bce_best:
            print(
                "Epoch {} improved from {} to {}".format(current_epoch, bce_best, bce)
            )
...
def train_epoch(
    ...
    for i, sample in pbar:
        ...
        if ohem:
            fake_loss = topk(fake_loss, k=min(ohem, fake_loss.size(0)), sorted=False)[
                0
            ].mean()
            real_loss = topk(real_loss, k=min(ohem, real_loss.size(0)), sorted=False)[
                0
            ].mean()
